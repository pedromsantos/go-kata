...
objective as `22_smellymarsrover`, but with a more everyday domain: a shopping
cart (inspired by `19_shoppingcart`), built across the same
...
Unlike `22_smellymarsrover`, which is a single-file fixture, this kata is
laid out across `domain.go`, `application.go`, and `infrastructure.go` so you
can practice all three test types on the same codebase: acceptance, unit, and
integration.

None of the `.go` files in this kata contain explanatory comments beyond
ordinary Go doc comments on exported identifiers. Finding and naming each
blocker and each smell is part of the exercise -- this README intentionally
...
| ------------------ | ------------------- | --------------------------------------------------------------------------- |
| Domain (`Cart`)     | mixed, 0%-partial   | no test deliberately targets the aggregate's core behavior                 |
| Application         | **0%** (`AddProductToCart`) | no tests at all for this use case                                  |
| Domain (services)   | tested               | tests exist in `domain_test.go`, but nearly all are smelly                 |
| Infrastructure      | tested               | tests exist in `infrastructure_test.go`, but nearly all are smelly         |
...
that make a type hard or impossible to put under test as-is. Before writing a
single test, identify each one and name it using a legacy-code testability
taxonomy of your choice (constructor does real work, hardcoded dependency,
singleton/global state, package-level function with a side effect,
non-determinism with no seam, etc.). There are more blockers than in
`22_smellymarsrover`, spread across two layers.
...
Note: Go has no runtime type-assertion escape hatch equivalent to
TypeScript's `as unknown as` cast, and no way to substitute a fake value
where a concrete `*Cart` aggregate is required. "Mocking Final/Concrete
Classes" and "saves a double instead of a real cart" therefore show up here
in their closest Go shape (see the porting notes in `ts-kata-answers` if you
want the details after attempting the kata yourself).

## Tasks

Pick one direction (or all three, on separate branches):

1. **Write the missing unit test, outside-in** -- `AddProductToCart` has no
   test. Decide what seam its hardcoded dependency (the product catalogue)
   needs before that's even possible.
2. **Fix the existing unit tests** -- everything in `domain_test.go`
...
3. **Fix the existing integration tests** -- same exercise, but for
   `infrastructure_test.go`. Pay attention to how the same smell can look
...
```sh
go test -v ./23_smellyshoppingcart/...
```
