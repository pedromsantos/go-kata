...
mission simulator (inspired by `11_marsrover` and `18_esamarsrover`) built
...
Neither `smellymarsrover.go` nor `smellymarsrover_test.go` contain any
explanatory comments. Finding and naming each blocker and each smell is part
of the exercise — this README intentionally does not say where they are.

## Current State

| Area                                                          | Coverage | What's wrong                                                 |
| -------------------------------------------------------------- | -------- | -------------------------------------------------------------- |
| `Rover`, `ObstacleSensor`, `RadioTransmitter`, `missionClockNow` | **0%**   | no tests at all                                               |
| `CommandTranslator`, `MissionLog`, `Coordinate`                 | tested   | tests exist in `smellymarsrover_test.go`, but nearly all of them are smelly |
...
The 0%-covered part of `smellymarsrover.go` has several deliberate testability
blockers — things that make a type hard or impossible to put under test as-is.
Before writing a single test, identify each one and name it using a
legacy-code testability taxonomy of your choice (constructor does real work,
hardcoded dependency, singleton/global state, package-level function with a
side effect, non-determinism with no seam, etc.).

The tests in `smellymarsrover_test.go` currently all pass, but the suite is
deliberately smelly. It represents every smell from this catalogue at least
once:
...
Note: Go has no `final`/sealed classes and no runtime type-assertion escape
hatch equivalent to TypeScript's `as unknown as` cast, so "Mocking Final /
Concrete Classes" shows up here in its closest Go shape: an interface
(`Translator`) introduced only so a test double can stand in for the concrete
`CommandTranslator`, then over-mocked.
...
2. **Fix the existing tests** — everything in `smellymarsrover_test.go`
